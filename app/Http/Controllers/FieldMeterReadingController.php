...
class FieldMeterReadingController extends Controller
{
    ...
    public function toAccounting($num)
    {
    ...
    public function store(Request $request)
    {
        ...
        if (isset($request->read_date)) {
            if (
                \Carbon\Carbon::parse($request->current_month) >= $request->read_date &&
    ...
    public function processWaterBill($request)
    {
        if (isset($request['read_date'])) {
            if (
                \Carbon\Carbon::parse($request['current_month']) >= $request['read_date'] &&
                ...
                return json_encode(['created' => false, 'msg' => 'Cannot create billing, make sure that the reading date is not covered from the previous reading date.']);
            }
        }

        if ($request['reading_meter'] < $request['meter_reading']) {
            return json_encode(['created' => false, 'msg' => 'Current meter reading should not be less than the previous meter reading.']);
        }
        ...
        $transactions = Transaction::create($fillable);
        return json_encode(['created' => true, 'data' => $fillable]);
    }
}
